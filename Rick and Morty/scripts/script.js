...
const headerContainer = crearElemento("div","container-fluid","")

root.appendChild(headerContainer)


const titleApi = crearElemento("h2","title-api","Rick and Morty API")
headerContainer.appendChild(titleApi)

const sectionsContainer = crearElemento("div","container","")
...
episodes.results.forEach((episode)=>{
    const episodio = crearElemento("div","individual-episode",episode.id +"-"+ episode.name)
...
const renderInfEpisode = (episode) =>{
    ...
    columnDer.innerHTML = `<h2>${episode.name}</h2><h5>${episode.air_date} | ${episode.air_date} </h5>`
}
...
const getCharacters = async (characters) => {
 ...
 const renderCharacters = (characterList) =>{
    const columnDer = document.querySelector(".panel-der")
    const charactersContainer = crearElemento("div","row,character-container","")
